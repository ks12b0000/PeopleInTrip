...
<body>
    ...
    <p class="cls1">찜한 내역(제주)</p>
	 ...
	 <br/>
    <hr/>
    <form action="${contextPath}/mypage/mypage_steamed_jeju_festival?id=${user.id}" method="post" name="show_MyTour_Steamed">
	    <div>
   	        <div class="category">
	            <a href="${contextPath}/mypage/mypage_steamed_jeju?id=${user.id}" style="color:#551A8B">여행지</a>
           	<a>축제</a>
           	<a href="${contextPath}/mypage/mypage_steamed_jeju_Exhibition?id=${user.id}" style="color:#551A8B">전시관</a>
	        </div>
	        <div align="center">
  	...
  	<div style="text-align: center; font-size: 18px;">		
		<c:if test="${pagingDTO.curPage > 1 }">
			<a href="${contextPath}/mypage/mypage_steamed_jeju_festival?id=${pagingDTO.id}&curPage=1" style="color: #9966ff; font-size: 25px;">&laquo;</a>
			<a href="${contextPath}/mypage/mypage_steamed_jeju_festival?id=${pagingDTO.id}&curPage=${pagingDTO.curPage-1 }" style="color: #9966ff; font-size: 25px;">&lt;</a>
		</c:if>
		<c:forEach begin="${pagingDTO.firstPage }"  end="${pagingDTO.lastPage }" var="i"> &nbsp;
	   		<a href="${contextPath}/mypage/mypage_steamed_jeju_festival?id=${pagingDTO.id}&curPage=${i }" style="font-size: 18px;">   
		...
		<c:if test="${pagingDTO.curPage < pagingDTO.totalPageCount }">
			<a href="${contextPath}/mypage/mypage_steamed_jeju_festival?id=${pagingDTO.id}&curPage=${pagingDTO.curPage+1 }" style="color: #9966ff; font-size: 25px;">&gt;</a>
			<a href="${contextPath}/mypage/mypage_steamed_jeju_festival?id=${pagingDTO.id}&curPage=${pagingDTO.totalPageCount }" style="color: #9966ff; font-size: 25px;">&raquo;</a>
		</c:if>
	</div>
