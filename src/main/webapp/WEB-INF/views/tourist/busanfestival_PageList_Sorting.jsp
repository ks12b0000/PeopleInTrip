		...
		<div id="tourPage_div">		
			...
			<c:forEach var="plist" items="${plist}">
				<table>
					<tr>
						...
						<td class="tb_td2">
							<span class="tourpost_title"><a href="${contextPath}/tourist/busanfestival_View?UC_SEQ=${plist.UC_SEQ}">${plist.PLACE}</a></span> <br> <strong>🧡 ${plist.steamedhit}개 💬 ${plist.commentcount}개 👍️ ${plist.suggestionhit}개 👀 ${plist.viewcount}회</strong> <br/>
			...
			<div name="tour_div3" id="tour_div3">
				<c:if test="${pagingDTO.curPage > 1 }">
					<a href="${contextPath}/tourist/busanfestival_PageList_Sorting?value=${value}&curPage=1" style="color: #9966ff; font-size: 25px;">&laquo;</a>
					<a href="${contextPath}/tourist/busanfestival_PageList_Sorting?value=${value}&curPage=${pagingDTO.curPage-1 }" style="color: #9966ff; font-size: 25px;">&lt;</a>
				</c:if>
					<c:forEach begin="${pagingDTO.firstPage }"  end="${pagingDTO.lastPage }" var="i"> &nbsp;
	   					<a href="${contextPath}/tourist/busanfestival_PageList_Sorting?value=${value}&curPage=${i }" style="font-size: 18px;"  >  
				...
				<c:if test="${pagingDTO.curPage < pagingDTO.totalPageCount }">
					<a href="${contextPath}/tourist/busanfestival_PageList_Sorting?value=${value}&curPage=${pagingDTO.curPage+1 }" style="color: #9966ff; font-size: 25px;">&gt;</a>
					<a href="${contextPath}/tourist/busanfestival_PageList_Sorting?value=${value}&curPage=${pagingDTO.totalPageCount }" style="color: #9966ff; font-size: 25px;">&raquo;</a>
				</c:if>
			</div>
		</div>
</body>
