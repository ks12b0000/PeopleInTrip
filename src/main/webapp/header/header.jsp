...
<link rel="stylesheet" href="../resources/css/header/header.css?ver123" />
...
<body>
	<header>
		<div class="logo">
			<a class="to_index" href="${contextPath}/mainpage/main"> People
				in Trip </a>
		</div>
		...
		<div class="search">
			<select name="searchType" id="selectBox">
				<option value="j"
					<c:out value="${scri2.searchType eq 'j' ? 'selected' : ''}"/>>제주</option>
				<option value="b"
					<c:out value="${scri2.searchType eq 'b' ? 'selected' : ''}"/>>부산</option>
		...
		<c:choose>
			<c:when test="${isLogIn==null}">
				<div class="container2">
					<div class="login">
						<a href="${contextPath}/login_signup/login">로그인</a>
					</div>
					<div class="join">
						<a href="${contextPath}/login_signup/signup_input">회원가입</a>
					</div>
				</div>
			</c:when>
			<c:otherwise>
				<div class="container2">
					<c:if test="${user.grade  == '일반회원' }">
						<div class="login">
							<a href="${contextPath}/mypage/mypage_renewal?id=${user.id}">마이페이지</a>
						</div>
					</c:if>
